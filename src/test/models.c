...
inline double GAF_CSA_R2(long double omega, float w1, float wr, long double taus, long double S2s, long double tauf, long double S2f, double D2) {
	printf("\t\t\tomega = %Lf, w1 = %f, wr = %f\n\t\t\ttaus = %Le, S2s = %Lf\n\t\t\ttauf = %Le, S2f = %Lf\n\t\t\tD2 = %f\n", omega, w1, wr, taus, S2s, tauf, S2f, D2);
	printf("\t\t\t%f\n", J0_EMF(omega, taus, S2s, tauf, S2f));
	return ( \
  		(1/45.) * (double) D2 * ( \
...
double GAF_13CR2(struct Residue *res, struct Relaxation* relax, long double taus, long double tauf, long double * sigs, long double * sigf) {
	...
	printf("\t[%f, %f, %f -> %f] // [%f, %f, %f, %f]\n", R2CSAx, R2CSAy, R2CSAxy, R2CSA, R2CH, R2CHr, R2CN, R2CC);
	printf("\t\tS2s [%f, %f, %f, %f, %f, %f]\n", S2CHs, S2CSAxs, S2CSAys, S2CSAxys, S2CNs, S2CCs);
	printf("\t\tS2f [%f, %f, %f, %f, %f, %f]\n", S2CHf, S2CSAxf, S2CSAyf, S2CSAxyf, S2CNf, S2CCf);
