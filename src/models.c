			...
			for (mp = -2; mp <= 2; mp++) {
				for (k = -2; k <= 2; k++) {
					for (kp = -2; kp <= 2; kp++) {
						...
						temp = cpowl(-1 * I, k - kp);
						expo = 0;
						expo = -(powl(sig[0], 2.) * (powl(k, 2) + powl(kp, 2)) / 2.);
						expo -= (powl(sig[1] * l, 2.));
						expo -= (powl(sig[2], 2.) * (powl(m, 2) + powl(mp, 2)) / 2.);
						temp *= expl(expo);
...
double GAF_15NR1(struct Residue *res, struct Relaxation* relax, long double taus, long double tauf, long double sigs[3], long double sigf[3]) {
	...
	d2x = powl(((csa[2] - csa[0]) * powl(10.,-6.)) * omega_15N, 2.);
	d2y = powl(((csa[1] - csa[0]) * powl(10.,-6.)) * omega_15N, 2.);
	d2xy= powl(powl(10.,-6.) * omega_15N, 2.) * (csa[2] - csa[0]) * (csa[1] - csa[0]);
	...
	R1NH = (0.1) * powl(D_NH, 2.) * (J0HC + 3 * J1 + 6 * J2HC);
	...
	R1NHr = (0.1) * powl(D_HNr, 2.) * (J0HC + 3 * J1 + 6 * J2HC);
	...
	R1CN = (0.1) * powl(D_CN, 2.) * (J0HC + 3 * J1 + 6 * J2HC);
...
double GAF_15NR2(struct Residue *res, struct Relaxation* relax, long double taus, long double tauf, long double sigs[3], long double sigf[3]) {
	...
	J0sum += (4/3.) * J0_EMF(2 * M_PI * (w1 - wr), &taus, &S2NHs, &tauf, &S2NHf);
	R2NH   = (1/20.) * powl(D_NH, 2.) * (J0sum + 3 * J1 + J0HC + 6 * J1H + 6 * J2HC);
	...
	J0sum += (4/3.) * J0_EMF(2 * M_PI * (w1 - wr), &taus, &S2CNs, &tauf, &S2CNf);
	R2NHr   = (1/20.) * powl(D_HNr, 2.) * (J0sum + 3 * J1 + J0HC + 6 * J1H + 6 * J2HC);
